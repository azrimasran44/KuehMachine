...
Sound follows the same retro-arcade logic: every cue is a small synthesized chiptune waveform (square/triangle/noise, hand-built the way the sprites are — no stock sound packs), not a recording. An 8-bit horror ambient bed (buzzy tritone drone, filtered static hiss, irregular unsettling blips and glitch stutters) plus sparse electrical clicks and the occasional distant thunder crack under the Start Screen's lightning strikes, a driving 8-bit horror chiptune loop under the story (walking-pace bass arpeggio, skittering off-beat lead, a Pokémon-style blip on every dialogue advance), and an upbeat driving track under gameplay, with a per-move "boink," a landing thud, and win/lose stingers layered on top — one shared audio system fades and ducks between all of it so nothing ever cuts off abruptly.
